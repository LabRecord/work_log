...
引言部分不同于摘要（Abstract）,字数上没有严格的局限性，但是内容方面要求在对应的选题上更加精准和明确（be concise and to the point）。引言通常需要承接摘要部分提出的几个学术方向展开话题，从比较宽泛的学术背景铺垫逐渐过渡到论文的研究课题（narrow down to the research problem）。
